...
def validate_research_report(report: dict) -> None:
    ...
    if report.get("schema_version") != ACCEPTANCE_SCHEMA:
        raise ValueError("研究验收 schema 不匹配")
...
def _validate_passed_evidence(report):
    CandidateLock.model_validate(report["selected_candidate"])
...
def validate_research_reproduce(value: dict) -> None:
    ...
    if value.get("schema") != REPRODUCE_SCHEMA:
        raise ValueError("研究复现 schema 不匹配")
    expected = {
        "schema",
        "protocol_hash",
        "candidate_lock_hash",
